...
import 'package:diplomaapp/pages/payment/subscription_plan_page.dart';
import '../mycourse/teacher_my_course_page.dart';

class TeacherSidebar extends StatefulWidget {
  final String selectedMenu;

  const TeacherSidebar({super.key, required this.selectedMenu});

  @override
  State<TeacherSidebar> createState() => _TeacherSidebarState();
}

class _TeacherSidebarState extends State<TeacherSidebar> with SingleTickerProviderStateMixin {
  ...
  void _onMenuSelect(String menuName) {
    ...
    if (menuName == "Schedule") {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => SubscriptionPlanPage()),
      );
    } else if (menuName == "My Courses") {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => TeacherMyCoursePage()),
      );
    }
  }
              ...
              children: [
                Tooltip(
                  ...
                  child: Icon(
                    icon,
                    color:
                        isSelected ? const Color(0xFF6A5AE0) : Colors.black54,
                  ),
                ),
                if (!_isCollapsed) ...[
